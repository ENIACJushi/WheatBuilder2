...
public:
    ...
    static void moveChunk() {
        ...
        auto& generateOption = command->setEnum("GenerateOption", { "generate" });
        ...
        command->mandatory("opEnum1", ParamType::Enum, generateOption, CommandParameterOption::EnumAutocompleteExpansion);
        ...
        command->mandatory("heightmap", ParamType::Bool);
        command->mandatory("save"     , ParamType::Bool);
        // 是否包含实体/群系在加载时决定
        command->addOverload({ saveOption, "start_x", "start_z", "end_x", "end_z", "dim_id", "file_name", "biome", "save" });
        command->addOverload({ loadOption, "start_x", "start_z", "dim_id", "file_name", "biome" });
        command->addOverload({ generateOption, "start_x", "start_z", "end_x", "end_z", "dim_id" });
        ...
        command->setCallback([](DynamicCommand const& command, CommandOrigin const& origin, CommandOutput& output, std::unordered_map<std::string, DynamicCommand::Result>& results) {
            ...
            case do_hash("save"): {
                ...
                bool biome = results["biome"].get<bool>();

                if (ChunkManager::startSaveAreaTask(start_x, start_z, end_x, end_z, file_name, dim_id, false, false, biome, save)) {
            ...
            case do_hash("load"): {
                ...
                ChunkManager::startLoadAreaTask(start_x, start_z, dim_id, file_name, false, false, biome);
                output.success("Start load area task");
                break;
            }
            case do_hash("generate") :{
                int start_x = results["start_x"].get<int>();
                int start_z = results["start_z"].get<int>();
                int end_x = results["end_x"].get<int>();
                int end_z = results["end_z"].get<int>();
                int dim_id = results["dim_id"].get<int>();
                if (ChunkManager::startGenerateAreaTask(start_x, start_z, end_x, end_z, dim_id)) {
                    output.success("Start generate area task");
                }
                else {
                    output.error("A generate task is running.");
                }
            }
            default:
                break;
    ...
    static void test() {
        ...
        // command->mandatory("mode", ParamType::Int);

        // command->addOverload({"mode"});
        // command->setCallback([](DynamicCommand const& command, CommandOrigin const& origin, CommandOutput& output, std::unordered_map<std::string, DynamicCommand::Result>& results) {
        //     bool result = true;
        //     int mode = results["mode"].get<int>();
        //     Dimension* dimension = Global<Level>->getDimension(AutomaticID<Dimension, int>(0));
        //     if (dimension != nullptr) {
        //         ChunkSource& chunkSource = dimension->getChunkSource();
        //         std::shared_ptr<class LevelChunk> levelChunk = chunkSource.getAvailableChunk(ChunkPos(0, 0));
        //         if (levelChunk != nullptr) {
        //             std::shared_ptr<class LevelChunk> newChunk =chunkSource.createNewChunk(ChunkPos(0, 0), ChunkSource::LoadMode(mode), false);
        //             // VanillaLevelChunkUpgrade::upgradeLevelChunk(*levelChunk, *newChunk, *Global<Level>->getBlockSource(0));
        //             // newChunk->setUnsaved();
        //         }
        //     }
